...
def execute_code_contests_reward(generated_code: str, public_tests: dict, timeout=5) -> float:
    """Sandbox for CodeContests (DeepMind)"""
    code = extract_code(generated_code)
    try:
...
def load_extended_code_datasets():
    ...
    # CodeContests replaces APPS (clean Parquet format)
    ds_cc = load_dataset("deepmind/code_contests", split="test")
    print(f"Loaded {len(ds_cc)} CodeContests problems (DeepMind Competitive)")
    
    return ds_lcb, ds_mbpp, ds_bcb, ds_cc
...
def run_phase6_zero_backprop():
    ...
    ds_lcb, ds_mbpp, ds_bcb, ds_cc = load_extended_code_datasets()
    ...
    def code_generator():
        combined = zip(ds_lcb, ds_mbpp, ds_bcb, ds_cc)
        for _ in range(start_step // 4): next(combined, None)
        for lcb, mbpp, bcb, cc in combined:
            yield ("lcb", lcb)
            yield ("mbpp", mbpp)
            yield ("bcb", bcb)
            yield ("cc", cc)
    ...
    for relative_step, (task_type, data) in enumerate(code_generator()):
        ...
        else: # cc (CodeContests)
            prompt = data["description"]
            sys_msg = "You are an elite competitive programmer. Write a Python script that reads from stdin and prints to stdout. Output Python code only."
        ...
        with torch.no_grad():
            ...
            max_tokens = 500 if task_type in ["lcb", "cc", "bcb"] else 200
        ...
        if task_type == "lcb": 
            ...
            reward = execute_code_contests_reward(generated_text, data["public_tests"])
        ...
        if (step + 1) % 10 == 0:
            ...
            print(f"  - CodeContests (Compet): {expert_usage['cc']} (Acc: {task_stats['cc']['success']}/{task_stats['cc']['attempts']})")
